...
In this tutorial, we will walk through a data science pipeline to learn some information from the data collected by the Counter Trafficking Data Collaborative, a collaborative effort that includes organizations like Polaris, the International Organization for Migration, and Liberty Shared.  
...
### 1.4 Tidying Data
Luckily for us, our dataset is quite neat which is often not the case. For this tutorial, we will only look at time, gender, age ranges, majority status when first registered and assisted, if the victim was sexually exploited, and different ways individuals were exploited for labor. We will trim down our dataset and only keep the columns we need. 
```{r drop_cols}
data_tab <- data %>%
  select("yearOfRegistration", "gender", "ageBroad", "majorityStatus", "isSexualExploit", "typeOfLabourAgriculture", "typeOfLabourAquafarming", "typeOfLabourBegging", "typeOfLabourConstruction", "typeOfLabourDomesticWork", "typeOfLabourHospitality", "typeOfLabourManufacturing", "typeOfLabourPeddling")
head(data_tab, n = 10)
```
...
Combined with the previous graph, we can see there is a steady increase in the number of reported female victims and a spike in cases in 2016. The spike may also reflect the fact that the data is incomplete, and there are a lot more unreported cases.

### 2.2 Comparing Gender in Different Labor Workforces
